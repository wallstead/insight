                          ...
                          $header = $response['headers']; // array of http header lines
                            ...
                            if( !empty( $array ) ) {
                                ...
                                foreach($array['response']['docs'] as $doc) {
                                    ...
                                    // $embed = wp_remote_get( 'http://search.issuu.com/api/2_0/document?q=username:insightunr&responseParams=%2A&sortBy=epoch' );
                                    $publicationName = $doc['docname'];
                                    if($counter == 0) {
                                        echo '<a href="'. esc_url( add_query_arg( 'p', $publicationName, site_url( '/reader/' ) ) ) .'"><div class="recent-journal index-'.$counter.'"><p><span>New</span> '.$docTitle.'</p><img src="https://image.isu.pub/'.$doc['documentId'].'/jpg/page_1_thumb_large.jpg" alt="'.$doc['title on Issuu'].'"></div></a>';// echo '<a href="https://issuu.com/insightunr/docs/'.$doc['docname'].'"><div class="recent-journal index-'.$counter.'"><p><span>New</span> '.$docTitle.'</p><img src="https://image.isu.pub/'.$doc['documentId'].'/jpg/page_1_thumb_large.jpg" alt="'.$doc['title on Issuu'].'"></div></a>';
                                    } else {
                                        echo '<a href="'. esc_url( add_query_arg( 'p', $publicationName, site_url( '/reader/' ) ) ) .'"><div class="recent-journal index-'.$counter.'"><p>'.$docTitle.'</p><img src="https://image.isu.pub/'.$doc['documentId'].'/jpg/page_1_thumb_large.jpg" alt="'.$doc['title on Issuu'].'"></div></a>';// echo '<a href="https://issuu.com/insightunr/docs/'.$doc['docname'].'"><div class="recent-journal index-'.$counter.'"><p>'.$docTitle.'</p><img src="https://image.isu.pub/'.$doc['documentId'].'/jpg/page_1_thumb_large.jpg" alt="'.$doc['title on Issuu'].'"></div></a>';
                                    }
