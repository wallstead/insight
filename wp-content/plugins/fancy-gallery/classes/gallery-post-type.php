...
abstract class Gallery_Post_Type {
  ...
  static function init(){
    add_Action('init', Array(static::class, 'registerPostType'), 10); # For the permalinks it is important that the post type is registered after the taxonomies
    add_Filter('post_updated_messages', Array(static::class, 'filterPostUpdateMessages'));
    add_Action(sprintf('save_post_%s', static::post_type_name), Array(static::class, 'savePost'), 10, 2);
    add_Filter(sprintf('manage_%s_posts_columns', static::post_type_name), Array(static::class, 'filterPostTypeColumns'));
    add_Action(sprintf('manage_%s_posts_custom_column', static::post_type_name), Array(static::class, 'filterPostTypeColumnValue'), 10, 2);
  }
  ...
  static function filterPostUpdateMessages($arr_message){
    return Array_Merge($arr_message, Array(static::post_type_name => Array(
  ...
  static function addMetaBoxes(){
    ...
    static::addMetaBox(I18n::t('Images'), Core::$plugin_folder . '/meta-boxes/images.php', 'normal', 'high');

    static::addMetaBox(__('Appearance'), Core::$plugin_folder . '/meta-boxes/appearance.php', 'normal', 'high');

    remove_Meta_Box('authordiv', static::post_type_name, 'normal');
    if (Current_User_Can($post_type_object->cap->edit_others_posts)){
      static::addMetaBox(I18n::t('Owner'), Core::$plugin_folder . '/meta-boxes/owner.php');
    }

    static::addMetaBox(I18n::t('Gallery Shortcode'), Core::$plugin_folder . '/meta-boxes/show-code.php', 'side', 'high');

    if (Options::get('lightbox'))
      static::addMetaBox(I18n::t('Gallery Hash'), Core::$plugin_folder . '/meta-boxes/show-hash.php', 'side', 'high');
    ...
    foreach (static::$arr_meta_box as $box_index => $meta_box){
      add_Meta_Box(
        ...
        $meta_box['title'],
        Array(static::class, 'printMetaBox'),
  ...
  static function printMetaBox($post, $box){
    $include_file = static::$arr_meta_box[$box['args']]['include_file'];
  ...
  static function updatePostTypeName(){
    global $wpdb;
    $wpdb->update($wpdb->posts, Array('post_type' => static::post_type_name), Array('post_type' => 'fancy_gallery'));
    $wpdb->update($wpdb->posts, Array('post_type' => static::post_type_name), Array('post_type' => 'fancy-gallery'));
	}
